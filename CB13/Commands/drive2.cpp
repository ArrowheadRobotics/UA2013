...
void drive2::Initialize() {
	...
	while (Robot::driver->en1->Get() > -520 || Robot::driver->en2->Get() > -520) {
...
void drive2::accelerate(float spd, float dist) {
	...
	while (Robot::driver->en1->GetDistance() > dist / 5 / -3
			|| Robot::driver->en2->GetDistance() > dist / 5 / -3) {

	}
	Robot::driver->spd1->Set(spd / 5 * 4);
	Robot::driver->spd2->Set(spd / 5 * 4);
	while (Robot::driver->en1->GetDistance() > dist / 5 / -2
			|| Robot::driver->en2->GetDistance() > dist / 5 / -2) {
